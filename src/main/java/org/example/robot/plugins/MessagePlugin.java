...
import org.example.robot.PluginHandler;
...
public class MessagePlugin extends EventPluginImpl{
    ...
    public void handle(){
        ...
        if(null != (pluginClazz = cmd2plugin1.get(cmd)))                // 优先查找插件指令
            ...
            if (!exc.get()) {
                ...
                cmd2plugin3.forEach(o -> {
                    if(!ret.get()) {
                        ret.set(pluginHandle(o, null, e));
                    }
                });
            }
        }
    ...
    private boolean pluginHandle(Object[] pluginInfo, List<String> cmds, MessageEvent event){
        try {
            ...
            // 获取 INSTANCE
            org.example.robot.plugins.msg.MessagePlugin plugin = clazz.newInstance();
    ...
    private void pluginHandle(Class<? extends MessagePluginImpl> pluginClazz, List<String> cmds, MessageEvent event){
        ...
        try {
            // 获取 INSTANCE
            org.example.robot.plugins.msg.MessagePlugin plugin = pluginClazz.newInstance();
    ...
    public EventType event() {
        return EventType.GroupMessageEvent;
    }
}
